...
def main():
    ...
    args = parser.parse_args()
    print(jax.devices())
    dataset = load_and_transform_odac25(args.traj)
    out_dir = train_utils.create_out_dir(config, tag="odac_c_allegro")
    print(f"Output directory: {out_dir}")
    dataset = scale_dataset(dataset, scale_R=0.1, scale_U=96.485, scale_e=11.7871, fractional=True)
    print(dataset.keys())
    print(dataset['box'][0])
    
    print(dataset['charge'][0])
    if  args.sl:
        small_idx, large_idx = identify_large_small_indices(dataset, max_atoms=300)
        dataset = filter_dataset(dataset, filter_options={"r_cutoff": 0.5, 'max_particles':1200}, fractional=True)
    ...
    nbrs_init, (max_neighbors, max_edges, avg_num_neighbors) = graphs.allocate_neighborlist(
        ...
        config["model"]["r_cutoff"], mask_key="mask", box_key="box",reps_key="reps",
    ...
    if args.sl:
            large_predictions = trainer_fm.predict(large_dataset, batch_size=config["optimizer"]["batch"] // len(jax.devices()))

            mae_U_large = onp.sqrt(onp.mean((large_predictions['U'] / onp.sum(large_dataset['mask'], axis=1) / 96.49 
                                            - large_dataset['U'] / onp.sum(large_dataset['mask'], axis=1) / 96.49) ** 2)) 
            print(f"Energy Large RMSE: {mae_U_large*1000:.1f} (meV/atom)")

            mae_C = onp.sqrt(onp.mean((large_predictions['charge'] / 11.7871 - large_dataset['charge'] / 11.7871) ** 2)) 
            print(f"Charge Large RMSE: {mae_C*1000:.1f} (me)")

            mae_F_large = onp.sqrt(onp.mean((large_predictions['F'] / 96.49 * 0.1 - large_dataset['F'] / 96.49 * 0.1) ** 2))
            print(f"Force Large RMSE: {mae_F_large*1000:.2f} (meV/A)")
