...
class AllegroQeq(hk.Module):
    ...
    def __init__(self,
        ...
        self.alpha = hk.get_parameter(
            "residual_alpha", shape=(), init=hk.initializers.Constant(0.0)#init=hk.initializers.Constant(0.0)
        )
...
class AllegroQeqLayer(hk.Module):
    ...
    def __init__(self,
        ...
        self.gammas_shift_scale = ScaleShiftLayer(2, 0.1)
        #self.a = hk.get_parameter("a", shape=[1], init=hk.initializers.Constant(20.0))
        #self.b = hk.get_parameter("b", shape=[1], init=hk.initializers.Constant(0.01))
    ...
    def __call__(
        ...
        outs = e3nn.haiku.MultiLayerPerceptron(
            [self.charge_embed_dim] * self.charge_embed_layers + [1,],
            self.mlp_activation,
            output_activation=False
        )(x)
        chis = outs[..., 0]  # First output variable
        

        # Compute gammas using the inverse relationship: gammas = a / (chis + b)
        gammas = 0.3 / (jax_nn.softplus(chis)  + 1e-3)  # Add epsilon to avoid division by zero
        ...
        #hardness = self.hardness[species]
