...
from chemutils.models.allegroQeq_mod_orig import model
#from chemutils.models.allegroQeq import model
...
class PaiNNLayer(hk.Module):
    ...
    def __init__(
        ...
        self.qeq_settings = qeq_settings
    ...
    def _message(
        ...
        ## TODO: Add qeq here
        if qeq is not None:
            print(f"Update messages with charge equilibration. ")

            vectors = e3nn.IrrepsArray("1o", dir_ij)

            (dds, _), qeq_out = model.AllegroQeqLayer(
                mlp_n_hidden=ds.shape[-1],
                **self.qeq_settings
            )(
                vectors, ds.squeeze(1), None, receivers,
                qeq["species"], qeq["charge_fn"]
            )
    ...
    def __call__(
        ...
        s, v = graph.nodes
        s, v = self._message(
            s, v, graph.edges, Wij, graph.senders, graph.receivers, qeq)
...
class PaiNN(hk.Module):
    ...
    def __init__(
        ...
        *args,
        ...
        eps: float = 1e-8,
        charge_embed_n_hidden: int = 128,
        charge_embed_n_layers: int = 2,
        envelope_p: int = 6,
        qeq: int = None,
        qeq_mp: bool = True,
        **kwargs,
        ...
        self._qeq_settings = qeq_settings

        self.cutoff_fn = layers.SmoothingEnvelope(envelope_p)
        ...
        if self._shared_interactions:
            assert qeq is None, "Cannot use Qeq with shared interactions."
            self.layers = [PaiNNLayer(hidden_size, 0, activation, eps=eps)] * n_layers
        else:
            self.layers = [
                PaiNNLayer(hidden_size, i, activation, eps=eps, qeq_settings=qeq_settings) for i in range(n_layers)
            ]
    ...
    def __call__(self, graph: jraph.GraphsTuple, charge_fn = None) -> Tuple[jnp.ndarray, jnp.ndarray]:
        """
        ...
        for n, layer in enumerate(self.layers):
            if self._qeq is not None and n == self._qeq:
                if self._qeq_mp:
                    graph = layer(graph, filter_list[n], qeq)
                    qeq_out = qeq["out"]
                else:
                    print(f"[PaiNN] Use CELLI to update hidden features")
                    rbf = self.radial_basis_fn(norm_ij).squeeze(1)

                    # Apply Qeq to update scalar features
                    x = graph.nodes.s.squeeze(1)
                    w = x[graph.senders] * self.cutoff_fn(norm_ij)
                    w = jnp.concat([w, rbf], axis=-1)

                    (dx, _), qeq_out = model.AllegroQeqLayer(
                        mlp_n_hidden=x.shape[-1],
                        **self._qeq_settings
                    )(
                        e3nn.IrrepsArray("1o", dir_ij), w, None,
                        graph.senders, qeq["species"], charge_fn
                    )

                    dx = jax.ops.segment_max(dx, graph.senders, x.shape[0])
                    x = jnp.clip(x + dx, -1e2, 1e2)
                    graph = graph._replace(
                        nodes=graph.nodes._replace(s=x[:, None, :])
                    )
...
painn_default_kwargs = OrderedDict(
    num_species=100,
    hidden_size=128,
    n_layers=4,
    n_rbf=20,
...
def painn_neighborlist_pp(displacement: space.DisplacementFn,
    ...
    def model(position: md_util.Array,
        ...
        charge_eq_energy = custom_electrostatics.charge_eq_energy_neighborlist(
            ...
            electrostatics=electrostatics, method=solver, grid=grid, alpha=alpha,
        ...
        features, qeq_out = net(graph, charge_fn=charge_qeq_fn)
        ...
        if mode in ["energy", "energy_and_charge"]:
            ...
            if qeq_out is None:
                ...
                print(f"[PaiNN] Return Qeq charges")
                charges, elec_pot = qeq_out

            if per_particle:
                return per_atom_energies
            else:
                return md_util.high_precision_sum(per_atom_energies), charges
