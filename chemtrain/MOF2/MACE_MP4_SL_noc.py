...
def preprocess_mof_data(data_path, train_cutoff=100, save_path = None):
    ...
    train_indices, temp_indices = train_test_split(indices, train_size=train_size, random_state=3)
    val_indices, test_indices = train_test_split(temp_indices, test_size=val_size, random_state=3)
...
def main():
    ...
    out_dir = train_utils.create_out_dir(config, tag="MOF_MP4_SL_noc")
    dataset, dataset_big = preprocess_mof_data(data_path, train_cutoff=100)
    dataset = process_dataset(dataset, charge="charge")
    print(f"Dataset sizes: {len(dataset['training']['R'])} training, {len(dataset['validation']['R'])} validation, {len(dataset['testing']['R'])} testing")
    print(f"Dataset R: {dataset['training']['R'].shape} large molecules")
