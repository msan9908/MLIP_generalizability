...
class C_LinearNodeEmbeddingLayer(hk.Module):
    def __init__(self, num_species: int, irreps_out: e3nn.Irreps):
        super().__init__()
        self.num_species = num_species
        self.irreps_out = e3nn.Irreps(irreps_out).filter("0e").regroup()
        self.embedding = hk.Embed(num_species,self.irreps_out.dim)
        self.tc_embedding = hk.Embed(9,128)

    def __call__(self, node_species: jnp.ndarray, tc: int) -> e3nn.IrrepsArray:
        embedding = self.embedding(node_species)
        tc_embedding = self.tc_embedding(tc)

        refined_node_features = embedding + hk.Linear(self.irreps_out.dim)(tc_embedding)#e3x.nn.add(e3x.nn.Dense(self.num_features)(refined_node_features), species_emb)
        ...
        refined_node_features = hk.Linear(self.irreps_out.dim)(refined_node_features)               # (N, num_features)
        refined_node_features = jax_nn.silu(refined_node_features)
        refined_node_features = hk.Linear(self.irreps_out.dim)(refined_node_features)
        return e3nn.IrrepsArray(self.irreps_out, refined_node_features)   
    
#TODO: Test modyfing LinearNodeEmbeddingLayer to use efa on embedding of efa is in input

class LinearNodeEmbeddingLayer_efa(hk.Module):
    def __init__(self, num_species: int, irreps_out: e3nn.Irreps):
        super().__init__()
        self.num_species = num_species
        self.irreps_out = e3nn.Irreps(irreps_out).filter("0e").regroup()
    ...
    def __call__(self, node_species: jnp.ndarray, efa: jnp.ndarray) -> e3nn.IrrepsArray:
        embedding = self.embedding(node_species)
        # Apply efa to the embedding
        if efa is not None:
            embedding = embedding + efa[:, jnp.newaxis]
        return e3nn.IrrepsArray(self.irreps_out, embedding)
...
class InteractionLayer(hk.Module):
    ...
    def __call__(
        ...
        )(vectors, node_feats, radial_embedding, senders, receivers, qeq)
...
class MessagePassingConvolutionLayer(hk.Module):
    ...
    def __call__(
        ...
        # Add the allegro QEQ layer here


        if qeq is not None:
            print(f"Apply QEQ during message passing")

            x = messages.filter(keep=["0e", "0o"]).array
            V = messages.filter(drop=["0e", "0o"])

            if self.qeq_params.get("aggregate_receivers", False):
                print(f"[Aggregate receiver messages]")
                dst = receivers
            else:
                dst = senders

            (dx, V), qeq_out = model.AllegroQeqLayer(
                mlp_n_hidden=x.shape[-1],
                **self.qeq_params
            )(
                vectors, x, V, dst, qeq["species"], qeq["charge_fn"],
            )

            # Residual update
            x += jax_nn.softplus(
                hk.get_parameter("q_alpha", (), jnp.float32, jnp.ones)
            ) * dx

            # Pass back the updated qeq
            qeq["out"] = qeq_out
            messages = e3nn.concatenate([x, V]).simplify()
