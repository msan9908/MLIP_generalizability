...
class EFAIntegrationBlock(flaxnn.Module):
    ...
    def __call__(self, species_emb, positions, batch_segments, graph_mask, i):
        ...
        if self.emulate_era_block:
            ...
            #debug.print("efa_input: {efa_input}", efa_input=self.era_max_length)
...
class Allegro(hk.Module):
    ...
    def __call__(
        ...
        refined_node_features = species_emb[:, None, None, :] + hk.Linear(self.embed_dim)(efa_node_features)#e3x.nn.add(e3x.nn.Dense(self.num_features)(refined_node_features), species_emb)
...
def efa_neighborlist_pp(displacement: space.DisplacementFn,
    ...
    def model(position: md_util.Array,
        ...
        #debug.print("real_position: {real_position}", real_position=real_position)
        ...
        net = Allegro(
            ...
            position=jnp.array(real_position, jnp.float32),
