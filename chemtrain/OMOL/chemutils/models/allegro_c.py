...
class AllegroQeq(hk.Module):
    ...
    def __init__(self,
        ...
        self.tc_embedding = hk.Embed(9, embed_dim)
    ...
    def __call__(
        ...
        #debug.print("tc: {}", self.total_charge)
        #debug.print("Embed: {}, Charge: {}", self.tc_embedding(self.total_charge), self.total_charge)

        refined_node_features = species_emb[:, None, None, :] + hk.Linear(self.embed_dim)(self.tc_embedding(self.total_charge))#e3x.nn.add(e3x.nn.Dense(self.num_features)(refined_node_features), species_emb)
        ...
        refined_node_features = hk.Linear(self.embed_dim)(refined_node_features)               # (N, num_features)
        refined_node_features = jax.nn.silu(refined_node_features)
        refined_node_features = hk.Linear(self.embed_dim)(refined_node_features)  # Final (N, num_features)
