...
def preprocess_mof_data(data_path, split_method="cluster", val_ratio=0.1, save_path=None):
    ...
    #valid_indices = dataset.pop('valid_indices')  # Original indices of valid samples

    # Scale and filter dataset
    ...
    name = scaled_dataset['names']
    all_names = scaled_dataset['names'].tolist()
    scaled_dataset.pop('names')
    #scaled_dataset = filter_dataset(scaled_dataset, filter_options={"r_cutoff": 0.5, 'max_particles':1000}, fractional=True)
    #scaled_dataset['names'] = name
    
    # Load splits using REFCODES instead of indices
    def load_refcode_splits(split_method):
        train_refcodes = onp.loadtxt(f'train_refcodes_{split_method}.csv', dtype=str).tolist()
        test_refcodes = onp.loadtxt(f'test_refcodes_{split_method}.csv', dtype=str).tolist()
        return train_refcodes, test_refcodes

    # Get refcodes for splits
    ...
    # Randomly shuffle training indices for validation split
    onp.random.seed(42)  # Consistent randomness
    shuffled_train = onp.random.permutation(train_indices)
    val_indices = shuffled_train[:num_val]
    train_indices = shuffled_train[num_val:].tolist()
    #scaled_dataset['names']

    # Create final splits with refcode validation
    dataset_splits = {
...
def main():
    ...
    out_dir = train_utils.create_out_dir(config, tag="Qeq_MOF_MACE_maxsep_noC")
    dataset = preprocess_mof_data(data_path, split_method="maxsep")
